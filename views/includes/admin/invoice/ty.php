<p style="margin: 5px 0 20px 0; text-decoration: underline; font-size: 10px; color: #000;">BS Business Travelling,Inh.O.Bilonozhko Marienwerderstr.11,30823 Garbsen</p>
...
<table cellpadding="0" width="100%" cellspacing="0" border="0" style="margin: 0px;margin-top: 75px;">
  <tr>
    <td align="left">
    ...
    <td style="vertical-align: top;" align="right">
      <p style="font-size: 13px;margin-top: 0px; margin-bottom: 0px;">Invoice Nr.: <?=$order['code'];?></p>
 ...
 <!-- <? if(isset($info['added_person']) && !empty($info['added_person'])):?>
      ...
      <? if(isset($info['added_person']) && !empty($info['added_person'])):?>
      	<? if(isset($info['breakfast']) && $info['breakfast'] == TRUE):?>
      		<p><?=number_format(($info['total_cost'] - $info['br_cost'] - $order['apartment']['add_person_out']*$info['nights'])/$info['nights'] + $order['apartment']['add_person_out'], 2, '.', '');?></p>
      	<? else:?>
      		<p><?=number_format(($info['total_cost'] - $order['apartment']['add_person_out']*$info['nights'])/$info['nights'] + $order['apartment']['add_person_out'], 2, '.', '');?></p>
      	<? endif;?>
      <? else:?>
      	<? if(isset($info['breakfast']) && $info['breakfast'] == TRUE):?>
      		<p><?=number_format(($info['total_cost'] - $info['br_cost'])/$info['nights'], 2, '.', '');?></p>
      	<? else:?>
      		<p><?=number_format($info['total_cost']/$info['nights'], 2, '.', '');?></p>
      	<? endif;?>
      <? endif;?>
...
<!--  <p><?=$info['apn_cost'];?></p> -->
      ...
      <? if(isset($info['added_person']) && !empty($info['added_person'])):?>
      	<? if(isset($info['breakfast']) && $info['breakfast'] == TRUE):?>
      		<p><?=number_format((($info['total_cost'] - $info['br_cost'] - $order['apartment']['add_person_out']*$info['nights'])/$info['nights'] + $order['apartment']['add_person_out'])*$info['nights'], 2, '.', '');?></p>
      	<? else:?>
      		<p><?=number_format((($info['total_cost'] - $order['apartment']['add_person_out']*$info['nights'])/$info['nights'] + $order['apartment']['add_person_out'])*$info['nights'], 2, '.', '');?></p>
      	<? endif;?>
      <? else:?>
      	<? if(isset($info['breakfast']) && $info['breakfast'] == TRUE):?>
      		<p><?=number_format($info['total_cost'] - $info['br_cost'], 2, '.', '');?></p>
      	<? else:?>
      		<p><?=number_format($info['total_cost'], 2, '.', '');?></p>
      	<? endif;?>
      <? endif;?>
  ...
  <tr>
    ...
    <td style="border-bottom: 1px solid #000;" align="center">
      <? if(isset($info['breakfast']) && $info['breakfast'] == TRUE):?>
        <p><?=number_format($info['br_cost'], 2, '.', '');?></p>
      <? endif;?>
    </td>
  </tr>
  <tr>
    <td style="border-bottom: 1px solid #000;" align="left">
      <? if($info['utype'] == 'fiz'): ?>
        <p>VAT 7% included </p>
      <? else: ?>
        <? if($info['vat_rate'] == '7_perc'): ?>
          <p>VAT 7% included </p>
        <? elseif($info['vat_rate'] == 'reverse_charge'):?>
          <p>VAT</p>
        <? elseif($info['vat_rate'] == 'zero'):?>
          <p>VAT</p>
        <? endif; ?>
      <? endif; ?>
    </td>
...
<table style="margin-top: 5px; border: 1px solid #000;" width="100%" height="25" cellpadding="0" cellspacing="0">
  <tr>
...
<table style="margin-top: 5px; margin-bottom: 10px; border: 0;" width="100%" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <td align="right" width="100%" style="vertical-align: top;">
      <p style="width: 200px; padding-right: 15px;"><?=convert_date($order['created_at'], 'm-d-Y');?> Hannover</p>
    </td>
  </tr>
</table>